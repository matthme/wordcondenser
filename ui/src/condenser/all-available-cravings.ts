...
export class AllAvailableCravings extends LitElement {
  ...
  renderList(availableCravings: Array<[DnaHash, [DnaRecipe, LobbyData[]]]>) {
    ...
    if (Object.values(availableCravings).length === 0) return html`
      <div class="column" style="justify-content: center; align-items: center; flex: 1;">
        <div style="color: #929ab9; margin-left: 20px; margin-top: 30px; font-size: 0.9em;">No available cravings found that you did not already join.</div>
      </div>`;
    ...
    return html`
      <div style="display: flex; flex-direction: row; flex-wrap: wrap;">
        ${availableCravings
          .sort(([_dnaHash_a, [dnaRecipe_a, _lobbyDatas_a]], [_dnaHash_b, [dnaRecipe_b, _lobbyDatas_b]]) => dnaRecipe_a.title.localeCompare(dnaRecipe_b.title))
          .map(([_dnaHash, [dnaRecipe, lobbyDatas]]) =>
            ...
            >
              <div class="column" style="align-items: flex-end; width: 100%;">
                <div class="row" style="margin-top: 5px; justify-content: flex-end; overflow-x: auto;">
                  ${
                    lobbyDatas.map((lobbyData) => {
                      if (lobbyData.info?.logo_src) {
                        return html`<img title=${lobbyData.name} src=${lobbyData.info.logo_src} style="height: 50px; width: 50px; border-radius: 50%; margin: 5px 2px 5px 2px;" />`
                      }
                  })
                  }
                </div>
              ...
              <div class="craving-title" style="margin-bottom: auto;">${dnaRecipe.title}</div>
              ...
              <!-- <div
                ...
                tabindex="0"
              >Install</div> -->
              <div class="column" style="align-items: flex-end; width: 100%; margin: 5px 0; margin-right: -15px;">
                <div
                  class="row confirm-btn"
                  style="align-items: center;"
                  @click=${async () => await this.joinCraving(dnaRecipe)}
                  @keypress=${async () => await this.joinCraving(dnaRecipe)}
                  tabindex="0"
                >
                  <span style="color: black; font-size: 23px; font-weight: 600; ${this.installing ? "opacity: 0.6" : ""}">
                    ${this.installing && dnaRecipe.resulting_dna_hash === this.installingDnaHash ? "installing..." : "Join"}
                  </span>
                </div>
              </div>
