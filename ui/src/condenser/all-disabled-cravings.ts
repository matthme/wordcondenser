...
export class AllDisabledCravings extends LitElement {
  ...
  renderList(cravings: Record<string, ClonedCell>) {
    ...
    if (Object.values(cravings).length === 0) return html`
      <div class="column" style="justify-content: center; align-items: center; flex: 1;">
        <div style="color: #929ab9; margin-left: 20px; margin-top: 30px; font-size: 0.9em;">No disabled cravings found.</div>
      </div>`;
    ...
    return html`
      <div style="display: flex; flex-direction: row; flex-wrap: wrap;">
        ${Object.entries(cravings)
          .sort(([cloneName_a, _cloneInfo_a], [cloneName_b, _cloneInfo_b]) => cloneName_a.localeCompare(cloneName_b))
          .map(([cloneName, cloneInfo]) =>
          html`
            ...
            >
              ...
              <img
                ...
                tabindex="0"
                @click=${async () => await this.enableCraving(cloneInfo.cell_id)}
                @keypress=${async () => await this.enableCraving(cloneInfo.cell_id)}
              />
            </div>
