...
import { resizeAndExport } from '../utils';
import { MVBButton } from '../components/mvb-button';

@customElement('create-lobby')
export class CreateLobby extends LitElement {
  ...
  @state()
  _unenforcedRules: string | undefined;
   ...
   * @internal
   */
  @query("#image-file-picker")
  private _imageFilePicker!: HTMLInputElement;


  isLobbyValid() {
    return true && this._name !== undefined && this._name !== ""
      && this._logoSrc !== undefined && this._logoSrc !== ""
      && this._description !== undefined && this._description != "";
  }


  onImageUploaded() {
    if (this._imageFilePicker.files && this._imageFilePicker.files[0]) {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          this._imageFilePicker.value = "";
        };
        img.src = e.target?.result as string;
      };
      reader.readAsDataURL(this._imageFilePicker.files[0]);
    }
  }
  ...
  async createLobby() {
    ...
    try {
      ...
      (this.shadowRoot?.getElementById("create-lobby-button") as MVBButton).disabled = false;
  ...
  render() {
    return html`
      ...
      <div style="display: flex; flex-direction: column; align-items: center;">
        ...
        <div class="box">
          <div style="font-size: 40px; font-weight: bold; color: #abb5da; opacity: 0.85; margin-bottom: 30px; margin-top: 40px;">Create New Group</div>
          <div style="font-size: 19px; line-height: 30px; color: #c5cded; margin-bottom: 50px; max-width: 800px; text-align: left;">
            This will create a <b>new peer-to-peer network</b> that keeps track of juicy cravings out in the wild.
            With <b>random secret words</b> that you can
            share with anyone you want to join.<br><br>#Holochain
            <span
          ...
          <div style="margin-bottom: 15px;">
            <mvb-textfield
              style="
                ...
                margin-bottom: 15px;
              "
              placeholder="Name of the Group (cannot be changed later)"
              @input=${(e: CustomEvent) => {
          ...
          <div style="margin-bottom: 15px;">
            <mvb-textarea
              ...
              placeholder="Description of the group. Can be changed later, but only by you."
              cols=57
              rows=6
              required
            ></mvb-textarea>
          ...
          <div style="margin-bottom: 30px;">
            <mvb-textarea
              ...
              placeholder="(Optional) Unenforced rules the group. Write anything that you would like people to abide by in this group. Can be changed later, but only by you."
              cols=57
              rows=6
              required
            ></mvb-textarea>
          ...
          <div style="font-size: 19px; line-height: 30px; color: #c5cded; margin-bottom: 20px; max-width: 800px; text-align: left;">
            Choose an image for this group (required):
          </div>
          <input
              type="file"
              ...
              style="display: none"
              accept="image/*"
              @change=${this.onImageUploaded}
          />
          <button @click=${() => this._imageFilePicker.click()}>Browse Files...</button>
          <div>
            ${
              this._logoSrc
                ? html`<img src=${this._logoSrc ? this._logoSrc : ""} style="height: 200px; width: 200px; border-radius: 40px; margin-top: 20px;"/>`
                : html``
            }
          </div>
          <!-- <div style="font-size: 19px; line-height: 30px; color: #c5cded; margin-bottom: 50px; margin-top: 30px; max-width: 800px; text-align: left;">
            The image can later be changed by anyone. It helps to navigate your groups and cravings.
          </div> -->
          ...
          <div style="margin-bottom: 10px; margin-top: 40px;">
            ...
            >
              <div class="row" style="align-items: center;">
                <span style="margin-left: 12px;">${this.installing ? "creating..." : "Create Group"}</span>
              </div>
            </mvb-button>
          </div>
