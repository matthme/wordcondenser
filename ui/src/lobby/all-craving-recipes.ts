...
import { clientContext, condenserContext, lobbyStoreContext } from '../contexts';
...
export class AllCravingRecipes extends LitElement {
  ...
  async installCraving(recipe: DnaRecipe) {
    ...
    try {
      ...
      this.dispatchEvent(new CustomEvent('craving-joined', {
        composed: true,
        bubbles: true,
        detail: {
          clonedCell,
  ...
  renderList(cravingRecipeRecords: Array<Record>) {
    ...
    let cravingRecipes: DnaRecipe[] = cravingRecipeRecords.map((record) => (decodeEntry(record) as DnaRecipe));
    ...
    return html`
      <div style="display: flex; flex-direction: column; margin: 8px;">
        ${cravingRecipes.map(
          (recipe) =>
            html`
              <div class="column" style="padding: 20px; border: 1px solid black; border-radius: 12px;">
                <div class="row" style="font-size: 18px; color: black;">
                  <div>
                    ${recipe.title}
                  </div>
                  <button @click=${async () => this.installCraving(recipe)}>Install</button>
                </div>
              </div>
            `
        )}
  ...
  render() {
    switch (this._allCravingRecipes.value.status) {
      ...
      case "complete":
        return html`
          <div>Secret words: <span style="background: grey; color: black; padding: 2px;">${(decodeEntry(this._store.lobbyInfo!) as LobbyInfo).network_seed}</span></div>
