...
export class AllLobbies extends LitElement {
  ...
  private _disabledLobbies = new StoreSubscriber(
    this,
    ...
    return html`
      <div style="display: flex; flex-direction: column; flex-wrap: wrap;">
        ${Array.from(lobbies.values())
          .sort(([store_a, _a, __a], [store_b, _b, __b]) => store_a.lobbyName.localeCompare(store_b.lobbyName))
          .map(([lobbyStore, _profilesStore, _dnaModifiers]) =>
          html`<lobby-detail .store=${lobbyStore}></lobby-detail>`
        )}

        <div style="color: #9098b3; margin-top: 80px; margin-bottom: 30px; ${Object.values(disabledLobbies).length === 0 ? "display: none;" : ""}">Disabled Groups:</div>
        ${Object.entries(disabledLobbies)
          .sort(([name_a, _a], [name_b, _b]) => name_a.localeCompare(name_b))
          .map(([name, cloneInfo]) =>
          html`<disabled-lobby-detail .name=${name} .cloneInfo=${cloneInfo}></disabled-lobby-detail>`
        )}
      </div>
