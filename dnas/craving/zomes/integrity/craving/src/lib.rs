    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                ...
                EntryTypes::Reflection(reflection) => {
                    validate_create_reflection(EntryCreationAction::Create(action), reflection)
                }
                EntryTypes::Association(association) => {
                    validate_create_association(EntryCreationAction::Create(action), association)
                }
                EntryTypes::Anecdote(anecdote) => {
                    validate_create_anecdote(EntryCreationAction::Create(action), anecdote)
                }
                EntryTypes::CommentOnOffer(comment_on_offer) => validate_create_comment_on_offer(
            ...
            OpEntry::UpdateEntry {
                ...
                EntryTypes::Reflection(reflection) => {
                    validate_create_reflection(EntryCreationAction::Update(action), reflection)
                }
                EntryTypes::Association(association) => {
                    validate_create_association(EntryCreationAction::Update(action), association)
                }
                EntryTypes::Anecdote(anecdote) => {
                    validate_create_anecdote(EntryCreationAction::Update(action), anecdote)
                }
                EntryTypes::CommentOnOffer(comment_on_offer) => validate_create_comment_on_offer(
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::CommentOnReflection(comment_on_reflection) => {
                        ...
                        let original_comment_on_reflection =
                            match CommentOnReflection::try_from(original_app_entry) {
                                Ok(entry) => entry,
                                Err(e) => {
                                    return Ok(ValidateCallbackResult::Invalid(format!(
                                        "Expected to get CommentOnReflection from Record: {e:?}"
                                    )));
                                }
                            };
                    ...
                    EntryTypes::CommentOnOffer(comment_on_offer) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_comment_on_offer =
                            match CommentOnOffer::try_from(original_app_entry) {
                                Ok(entry) => entry,
                                Err(e) => {
                                    return Ok(ValidateCallbackResult::Invalid(format!(
                                        "Expected to get CommentOnOffer from Record: {e:?}"
                                    )));
                                }
                            };
                    ...
                    EntryTypes::Anecdote(anecdote) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_anecdote = match Anecdote::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Anecdote from Record: {e:?}"
                                )));
                            }
                        };
                    ...
                    EntryTypes::Association(association) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_association = match Association::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Association from Record: {e:?}"
                                )));
                            }
                        };
                    ...
                    EntryTypes::Reflection(reflection) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_reflection = match Reflection::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Reflection from Record: {e:?}"
                                )));
                            }
                        };
                    ...
                    EntryTypes::Offer(offer) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_offer = match Offer::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get Offer from Record: {e:?}"
                                )));
                            }
                        };
                        validate_update_offer(action, offer, original_create_action, original_offer)
                    }
                }
            }
            _ => Ok(ValidateCallbackResult::Valid),
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                ...
                EntryTypes::Reflection(reflection) => validate_delete_reflection(
                    delete_entry.clone().action,
                    original_action,
                    reflection,
                ),
                EntryTypes::Association(association) => validate_delete_association(
                    delete_entry.clone().action,
                    original_action,
                    association,
                ),
                EntryTypes::Anecdote(anecdote) => {
                    validate_delete_anecdote(delete_entry.clone().action, original_action, anecdote)
                }
                EntryTypes::CommentOnOffer(comment_on_offer) => validate_delete_comment_on_offer(
                    delete_entry.clone().action,
                    original_action,
                    comment_on_offer,
                ),
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::OfferUpdates => {
                validate_create_link_offer_updates(action, base_address, target_address, tag)
            }
            LinkTypes::ReflectionUpdates => {
                validate_create_link_reflection_updates(action, base_address, target_address, tag)
            }
            LinkTypes::AssociationUpdates => {
                validate_create_link_association_updates(action, base_address, target_address, tag)
            }
            LinkTypes::AnecdoteUpdates => {
                validate_create_link_anecdote_updates(action, base_address, target_address, tag)
            }
            LinkTypes::OfferToCommentOnOffers => validate_create_link_offer_to_comment_on_offers(
                action,
                base_address,
                target_address,
                tag,
            ),
            LinkTypes::CommentOnOfferUpdates => validate_create_link_comment_on_offer_updates(
                action,
                base_address,
                target_address,
                tag,
            ...
            LinkTypes::EntryToResonator => {
                validate_create_link_entry_to_resonator(action, base_address, target_address, tag)
            }
            ...
            LinkTypes::AllAssociations => {
                validate_create_link_all_associations(action, base_address, target_address, tag)
            }
            LinkTypes::AllReflections => {
                validate_create_link_all_reflections(action, base_address, target_address, tag)
            }
            LinkTypes::AllAnecdotes => {
                validate_create_link_all_anecdotes(action, base_address, target_address, tag)
            }
        },
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::OfferUpdates => validate_delete_link_offer_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::ReflectionUpdates => validate_delete_link_reflection_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AssociationUpdates => validate_delete_link_association_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AnecdoteUpdates => validate_delete_link_anecdote_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::OfferToCommentOnOffers => validate_delete_link_offer_to_comment_on_offers(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::CommentOnOfferUpdates => validate_delete_link_comment_on_offer_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::EntryToResonator => validate_delete_link_entry_to_resonator(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AllAssociations => validate_delete_link_all_associations(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AllReflections => validate_delete_link_all_reflections(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AllAnecdotes => validate_delete_link_all_anecdotes(
                action,
                original_action,
                base_address,
                target_address,
        ...
        FlatOp::StoreRecord(store_record) => match store_record {
                ...
                match app_entry {
                    EntryTypes::Offer(offer) => {
                        ...
                        if let ValidateCallbackResult::Valid = result {
                            ...
                            validate_update_offer(action, offer, original_action, original_offer)
                        } else {
                            Ok(result)
                        }
                    }
                    ...
                    EntryTypes::CommentOnReflection(comment_on_reflection) => {
                        ...
                        if let ValidateCallbackResult::Valid = result {
                            let original_comment_on_reflection: Option<CommentOnReflection> =
                                original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                            let original_comment_on_reflection =
                                match original_comment_on_reflection {
            ...
            OpRecord::DeleteEntry {
                ...
                match original_app_entry {
                    ...
                    EntryTypes::Reflection(original_reflection) => {
                        validate_delete_reflection(action, original_action, original_reflection)
                    }
                    EntryTypes::Association(original_association) => {
                        validate_delete_association(action, original_action, original_association)
                    }
                    EntryTypes::Anecdote(original_anecdote) => {
                        validate_delete_anecdote(action, original_action, original_anecdote)
                    }
                    ...
                    EntryTypes::CommentOnReflection(original_comment_on_reflection) => {
                        validate_delete_comment_on_reflection(
                            action,
                            original_action,
                            original_comment_on_reflection,
            ...
            OpRecord::CreateLink {
                ...
                LinkTypes::ReflectionUpdates => validate_create_link_reflection_updates(
                    action,
                    base_address,
                    target_address,
                    tag,
                ),
                LinkTypes::AssociationUpdates => validate_create_link_association_updates(
                    action,
                    base_address,
                    target_address,
                    tag,
                ...
                LinkTypes::EntryToResonator => validate_create_link_entry_to_resonator(
                    action,
                    base_address,
                    target_address,
                    tag,
                ),
                LinkTypes::ActionToResonator => validate_create_link_action_to_resonator(
                    action,
                    base_address,
                    target_address,
                    tag,
