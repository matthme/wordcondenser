...
pub fn create_lobby_info(lobby_info: ZomeFnInput<LobbyInfo>) -> ExternResult<Record> {
    ...
    create_link(
        anchor,
        lobby_info_hash.clone(),
        LinkTypes::AnchorToLobbyInfo,
        (),
    )?;
    let record = get(lobby_info_hash, GetOptions::default())?.ok_or(wasm_error!(
...
pub fn get_lobby_info(input: ZomeFnInput<()>) -> ExternResult<Option<Record>> {
    ...
    let original_lobby_info_hash =
        ActionHash::try_from(anchor_links.first().unwrap().target.clone())
...
pub fn update_lobby_info(input: UpdateLobbyInfoInput) -> ExternResult<Record> {
    ...
    let record =
        get(updated_lobby_info_hash.clone(), GetOptions::default())?.ok_or(wasm_error!(
...
pub fn get_lobby_name(_: ()) -> ExternResult<String> {
    let dna_info = dna_info()?;
    let properties =
        LobbyDnaProperties::try_from(dna_info.modifiers.properties).map_err(|err| {
            wasm_error!(WasmErrorInner::Guest(format!(
                "Failed to read lobby dna properties from dna info: {}",
                err.to_string()
            )))
        })?;
    Ok(properties.name)
