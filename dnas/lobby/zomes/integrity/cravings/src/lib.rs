    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                EntryTypes::DnaRecipe(dna_recipe) => {
                    validate_create_dna_recipe(EntryCreationAction::Create(action), dna_recipe)
                }
                EntryTypes::LobbyInfo(lobby_info) => {
                    validate_create_lobby_info(EntryCreationAction::Create(action), lobby_info)
                }
            },
            OpEntry::UpdateEntry {
                app_entry, action, ..
                ...
                EntryTypes::DnaRecipe(dna_recipe) => {
                    validate_create_dna_recipe(EntryCreationAction::Update(action), dna_recipe)
                }
                EntryTypes::LobbyInfo(lobby_info) => {
                    validate_create_lobby_info(EntryCreationAction::Update(action), lobby_info)
                }
            },
            ...
            OpUpdate::Entry { app_entry, action } => {
                ...
                match app_entry {
                    EntryTypes::LobbyInfo(lobby_info) => {
                        ...
                        let original_lobby_info = match LobbyInfo::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get LobbyInfo from Record: {e:?}"
                                )));
                            }
                        };
                    ...
                    EntryTypes::DnaRecipe(dna_recipe) => {
                        let original_app_entry =
                            must_get_valid_record(action.clone().original_action_address)?;
                        let original_dna_recipe = match DnaRecipe::try_from(original_app_entry) {
                            Ok(entry) => entry,
                            Err(e) => {
                                return Ok(ValidateCallbackResult::Invalid(format!(
                                    "Expected to get DnaRecipe from Record: {e:?}"
                                )));
                            }
                        };
        ...
        FlatOp::RegisterDelete(delete_entry) => {
            ...
            match original_app_entry {
                EntryTypes::DnaRecipe(dna_recipe) => validate_delete_dna_recipe(
                    delete_entry.clone().action,
                    original_action,
                    dna_recipe,
                ),
                EntryTypes::LobbyInfo(lobby_info) => validate_delete_lobby_info(
                    delete_entry.clone().action,
                    original_action,
                    lobby_info,
                ),
        ...
        FlatOp::RegisterCreateLink {
            ...
            LinkTypes::AllCravingRecipes => {
                validate_create_link_all_craving_recipes(action, base_address, target_address, tag)
            }
            LinkTypes::LobbyInfoUpdates => {
                validate_create_link_lobby_info_updates(action, base_address, target_address, tag)
            }
            LinkTypes::AnchorToLobbyInfo => validate_create_link_anchor_to_lobby_infos(
                action,
                base_address,
                target_address,
                tag,
        ...
        FlatOp::RegisterDeleteLink {
            ...
            LinkTypes::AllCravingRecipes => validate_delete_link_all_craving_recipes(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::LobbyInfoUpdates => validate_delete_link_lobby_info_updates(
                action,
                original_action,
                base_address,
                target_address,
            ...
            LinkTypes::AnchorToLobbyInfo => validate_delete_link_anchor_to_lobby_infos(
                action,
                original_action,
                base_address,
                target_address,
            ...
            OpRecord::DeleteEntry {
                ...
                match original_app_entry {
                    EntryTypes::DnaRecipe(original_dna_recipe) => {
                        validate_delete_dna_recipe(action, original_action, original_dna_recipe)
                    }
                    EntryTypes::LobbyInfo(original_lobby_info) => {
                        validate_delete_lobby_info(action, original_action, original_lobby_info)
                    }
                }
            }
            OpRecord::CreateLink {
                ...
                LinkTypes::AllCravingRecipes => validate_create_link_all_craving_recipes(
                    action,
                    base_address,
                    target_address,
                    tag,
                ),
                LinkTypes::LobbyInfoUpdates => validate_create_link_lobby_info_updates(
                    action,
                    base_address,
                    target_address,
                    tag,
